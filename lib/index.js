  ...
  try {
    ...
    if (requestType === "GET") {
      if (req.body.DatabaseName === "mongo") {
        const compKeys = req.body.CompulsoryKeys || {};
        const tempKeys = req.body.TempKeys || {};
        const query = MongoQueryDa(compKeys, tempKeys);
        const data =
          req.body.getNumberOfResults === "all"
      ...
      if (req.body.DatabaseName === "mongo") {
        const CompulsoryKeys = req.body.CompulsoryKeys || {}; // Ensure it defaults to an empty object
        const TempKeys = req.body.TempKeys || {}; // Ensure it defaults to an empty object
